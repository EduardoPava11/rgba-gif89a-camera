...
class M2Processor {
    ...
    suspend fun processSession(
        ...
        frames.forEachIndexed { index, frameData ->
            try {
                ...
                // Save as JPEG with exact naming convention (better compatibility than PNG)
                ...
                // Store bitmap for mosaic generation and M3 pipeline
        ...
        if (AppConfig.useM3GifExport && processedBitmaps.size == AppConfig.CAPTURE_FRAME_COUNT) {
            try {
        ...
        val timingStats = try { `getM2TimingStats`() } catch (e: Exception) { null }
        val qualityMetrics = try { `getM2QualityMetrics`() } catch (e: Exception) { null }
        ...
        Log.i(TAG, "Average per frame: ${avgTime}ms, Neural confidence: ${qualityMetrics?.`policyConfidenceAvg` ?: "N/A"}")
    ...
    private fun generateTimingLogs(sessionDir: File) {
        ...
        if (timingStats == null) {
            timingFile.writeText("M2 timing logs unavailable - UniFFI error\n")
        ...
        timingFile.writeText(buildString {
            // Per-frame timing
            timingStats.`perFrameTimings`.forEachIndexed { index, timing ->
                appendLine("Frame %03d: %.1fms (policy: %.1fms, kernel: %.1fms, output: %.1fms)"
                    .format(index, timing, timing * 0.24, timing * 0.48, timing * 0.28))
            }
            
            appendLine("---")
            appendLine("Total M2 Duration: %.1fs".format(timingStats.`totalDurationMs`.toDouble() / 1000.0))
            appendLine("Average Per Frame: %.1fms".format(timingStats.`avgFrameMs`))
            appendLine("Min Frame Time: %.1fms".format(timingStats.`minFrameMs`))
            appendLine("Max Frame Time: %.1fms".format(timingStats.`maxFrameMs`))
            appendLine("Frames Processed: ${timingStats.`framesProcessed`}")
            
            // Simulated breakdown (in real implementation, these would come from Rust)
            appendLine("Policy Head Avg: %.1fms".format(timingStats.`avgFrameMs` * 0.24))
            appendLine("Kernel Selection Avg: %.1fms".format(timingStats.`avgFrameMs` * 0.48))
            appendLine("Output Generation Avg: %.1fms".format(timingStats.`avgFrameMs` * 0.28))
        })
    ...
    private fun generateQualityReport(sessionDir: File, successCount: Int) {
        ...
        if (timingStats == null || qualityMetrics == null) {
            reportFile.writeText("{\n  \"error\": \"M2 quality metrics unavailable - UniFFI error\"\n}")
            return
        }
        ...
        val report = M2QualityReport(
            ...
            processingStats = ProcessingStats(
                totalDurationMs = timingStats.`totalDurationMs`,
                avgFrameMs = timingStats.`avgFrameMs`,
                minFrameMs = timingStats.`minFrameMs`,
                maxFrameMs = timingStats.`maxFrameMs`
            ),
            qualityMetrics = QualityMetricsJson(
                avgSsim = qualityMetrics.`avgSsim`,
                avgPsnr = qualityMetrics.`avgPsnr`,
                edgePreservation = qualityMetrics.`edgePreservation`
            ),
            neuralStats = NeuralStats(
                policyConfidenceAvg = qualityMetrics.`policyConfidenceAvg`,
                valuePredictionAvg = qualityMetrics.`valuePredictionAvg`,
                kernelDiversity = qualityMetrics.`kernelDiversity`
            )
        )
    ...
    /**
     * Save bitmap as JPEG (better compatibility and smaller size than PNG)
     */
